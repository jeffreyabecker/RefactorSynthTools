...
// };
    ...
    memcpy(&setting->adsr_mod_f, &adsr_def, sizeof(adsr_def));
        ...
        if (voice->active)
        {
            ...
            for (uint32_t n = 0; n < len; n++)
            {
                if (n % 4 == 0)
                {
                    ...
                    (void)ASRM_Process(&voice->cfg->adsr_mod_f, &voice->adsr_mod_f);
                ...
                if (n % 32 == 0)
                {
                    ...
                    ff += voice->cfg->modulation * voice->adsr_mod_f.ctrl * voice->cfg->adsr_mod_f.s ;
                    if (ff > 1.0f)
                    {
                        ff = 1.0f;
                    }
...
inline void Synth_NoteOn(uint8_t ch, uint8_t note, float vel __attribute__((unused)))
{
    ...
    ADSR_Start(&voice->cfg->adsr_mod_f, &voice->adsr_mod_f);
    ...
    ASRM_Process(&voice->cfg->adsr_mod_f, &voice->adsr_mod_f);
